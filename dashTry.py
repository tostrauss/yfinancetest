...
# Data Fetching via yfinance (always using yfinance)
...
# For the Metrics Tracker, we base recommendations solely on RSI.
...
# -------------------------------
# DASH APP & LAYOUT SETUP (Using Bootstrap for improved styling)
# -------------------------------
...
# We use dcc.Store to hold watchlist, SMTP settings, and temporary options data.
store_components = [
...
header = dbc.Navbar(
    dbc.Container(
        [
            dbc.NavbarBrand("ToFu´s Stock Analysis & Options Trading", className="ms-2"),
        ]
    ),
    color="primary",
    dark=True,
...
footer = html.Footer(
    dbc.Container(
        html.P("© 2025 Tobias Strauss", className="text-center text-muted my-2")
    ),
    style={
        "position": "fixed",
        "left": "0",
...
# Define the layout using dbc.Tabs for multi-page navigation
...
# -------------------------------
# CALLBACKS FOR STOCK ANALYSIS TAB
# -------------------------------
...
def update_option_chain(expiration, ticker):
    ...
    if not calls.empty:
        ...
        fig.add_trace(go.Scatter(x=calls_sorted["strike"], y=calls_sorted["BS_Price"], mode="markers+lines", name="Calls", marker=dict(color="blue")), row=1, col=1)
    ...
    if not puts.empty:
        ...
        fig.add_trace(go.Scatter(x=puts_sorted["strike"], y=puts_sorted["BS_Price"], mode="markers+lines", name="Puts", marker=dict(color="red")), row=2, col=1)
...
@app.callback(
    ...
    [State("smtp-server", "value"), State("smtp-port", "value"), State("smtp-user", "value"), State("smtp-password", "value")],
)
def save_smtp(n_clicks, server, port, user, password):
    if n_clicks is None or n_clicks < 1:
        return ""
    smtp_settings = {"server": server, "port": port, "user": user, "password": password}
...
# -------------------------------
# COMBINED CALLBACK FOR NOTIFICATION SUBSCRIPTION
# -------------------------------

@app.callback(
    Output("notification-status", "children"),
    [Input("subscribe-button", "n_clicks"),
     Input("test-notify-button", "n_clicks")],
    [State("notify-email", "value"),
     State("notify-ticker", "value"),
     State("notify-period", "value"),
     State("notify-interval", "value"),
...
@app.callback(
    ...
    [State("option-call-ticker", "value"),
     State("option-call-S", "value"),
     State("option-call-K", "value"),
     State("option-premium", "value"),
     State("days-to-expiration", "value"),
     State("risk-free-rate", "value"),
     State("implied-vol", "value"),
     State("num-contracts", "value")]
)
...
@app.callback(
    ...
    [State("trade-type", "value"), State("entry-price", "value"), State("stop-loss", "value"), State("target-price", "value"), State("position-size", "value")],
)
